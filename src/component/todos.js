...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      ...
      var due_date = new Date(todo.due)
      
      if (due_date < dateToday) {color = "red"}
        ...
        <Grid key={todo.id}>
          <Card style={{ marginTop: 10, background: color }} data-testid={todo.content}>
            ...
            <ListItemButton component="a" href="#simple-list">
              <Checkbox data-testid={todo.content + "_task"} style={{paddingLeft:0}} color="primary" onClick={() => deleteTodo(todo.id)}/>
